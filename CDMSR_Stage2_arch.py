...
import CDMSR.archs.attention as attention
import torch
...
class MPU(nn.Module):
    ...
    def forward(self, x):
        ...
        # 逐元素相加
        out = branch_3x3 + branch_5x5 + branch_7x7
...
class DDGM(nn.Module):
    def __init__(self, in_channels, cond_dim=256, hidden_channels=None, reduction=16, dw=1, window_size=None,
        ...
        self.kernel = nn.Sequential(
            nn.Linear(cond_dim, in_channels * 2, bias=False),
        )
...
class HSSM(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias, LayerNorm_type, input_resolution):
        ...
        # 使用DDGM类，设置hidden_channels为扩展后的维度
        self.ffn = DDGM(
            in_channels=dim,
    ...
    def forward(self, y):
        ...
        norm_x = self.norm1(x)
        # 转换为序列 (B, C, H, W) -> (B, L, C)
        x_seq = norm_x.flatten(2).permute(0, 2, 1)  # TODO
        # 通过ResidualGroup，faltten(2)表示从第二个维度开始展平，permute表示将展平后的张量第一维与第二维换位置
        res_group_out = self.residual_group(x_seq, (H, W))
        # 转换回特征图 (B, L, C) -> (B, C, H, W)
        res_group_out = res_group_out.permute(0, 2, 1).view(B, C, H, W)
        x = x + res_group_out  # TODO

        # 第二部分：FFN处理
        norm2_x = self.norm2(x)
        # 注意参数顺序：P(条件)在前，I(图像)在后
        ffn_out = self.ffn(k_v, norm2_x)
        x = x + ffn_out
...
class HSSMNet(nn.Module):
    ...
    def forward(self, inp_img, k_v):
        ...
        inp_dec_level3 = torch.cat([inp_dec_level3, out_enc_level3], 1)  # add channel attention?
        inp_dec_level3 = self.ca4_3(inp_dec_level3)  # feture fusion
        inp_dec_level3 = self.reduce_chan_level3(inp_dec_level3)
        out_dec_level3, _ = self.decoder_level3([inp_dec_level3, k_v])

        inp_dec_level2 = self.up3_2(out_dec_level3)
        inp_dec_level2 = torch.cat([inp_dec_level2, out_enc_level2], 1)
        inp_dec_level2 = self.ca3_2(inp_dec_level2)  # feture fusion
        inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)
        out_dec_level2, _ = self.decoder_level2([inp_dec_level2, k_v])

        inp_dec_level1 = self.up2_1(out_dec_level2)
        inp_dec_level1 = torch.cat([inp_dec_level1, out_enc_level1], 1)
        inp_dec_level1 = self.ca2_1(inp_dec_level1)  # feture fusion
        ...
        # out_dec_level1 = self.tail(out_dec_level1) + self.pixelshuffle_inp(inp_img)

        return out_dec_level1


# PFEM_s2: 第二阶段使用的PFEM
class PFEM_s2(nn.Module):
    def __init__(self, n_feats=64, n_encoder_res=6, scale=4):
        super(PFEM_s2, self).__init__()
        self.scale = scale
        if scale == 2:
    ...
    def forward(self, x):  # stage2:just LR
        if self.scale == 2:
...
class CDMSRNet2(nn.Module):
    def __init__(self,
        ...
        self.condition = PFEM_s2(n_feats=64, n_encoder_res=n_encoder_res, scale=scale)
    ...
    def forward(self, img, IPF=None):
        if self.training:
            IRR, pred_IRR_list = self.diffusion(img, IPF)
            sr = self.G(img, IRR)
            return sr, pred_IRR_list
        else:
            IPF = self.diffusion(img)
            sr = self.G(img, IPF)
            return sr
